...
def mode_match_rows(results_df):
...
def build_evaluation_payload(analysis_folder):
    ...
    if not results_path.exists() or not report_path.exists() or not config_path.exists():
        raise FileNotFoundError(
            f"Missing required files in {folder}: results.csv, stats/comprehensive_report.csv, analysis_config.json"
        )

    results_df = pd.read_csv(results_path)
    report_df = pd.read_csv(report_path)
    config = load_json(config_path)
    mode_df, recovered_counts, recovered_names = mode_match_rows(results_df)
    best_df = best_reconstruction_rows(report_df)
