...
**Time Mask:** Αν χρειάζεται διαφορετικό παράθυρο, δίνεται `--time-start` ή/και `--time-end`. Για παράδειγμα, `python IEEE39/analyze_ieee39.py --scenario load29 --time-start 0.4 --time-end 20` κρατά μόνο τις γραμμές από `0.4s` έως `20s`. Αν πρέπει να διατηρηθούν οι αρχικοί χρόνοι του CSV χωρίς μετατόπιση, δίνεται `--no-reset-time`.
