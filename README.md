...
**Χρόνος Load Event:** Από προεπιλογή το load event τοποθετείται στο `t=0`, αλλά πλέον μπορεί να αλλάξει από command line με `--event-time`. Για παράδειγμα, `python IEEE39/generate_data.py --scenario load03 --event-time 0.5` δημιουργεί τα δεδομένα με το event στο `0.5s`. Όταν το event time είναι διαφορετικό από το default, προστίθεται suffix τύπου `_evt0.5s` στο όνομα του scenario folder, ώστε να ξεχωρίζουν τα runs.
