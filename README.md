...
**Ανάλυση Δεδομένων:** Το αρχείο `IEEE39/analyze_ieee39.py` διαβάζει τα `g*.csv` από το `IEEE39/results` και γράφει τα αποτελέσματα στο `IEEE39/analysis`. Το `--scenario` είναι πλέον υποχρεωτικό για κανονικό run, ώστε το script να μη ξεκινά σιωπηλά `all` όταν δοθεί σκέτο `python IEEE39/analyze_ieee39.py`. Το `--scenario` δέχεται περισσότερες από μία μορφές input: preset aliases όπως `load29`, πολλαπλά aliases όπως `load03 load24`, το ειδικό `all`, ή ακριβές folder name από το `IEEE39/results`, π.χ. `Load29_Pplus2_50s`. Τα default keys όπως `load29` δείχνουν στα προκαθορισμένα σενάρια `Pplus2`, άρα το `load29` αντιστοιχεί στο input `IEEE39/results/Load29_Pplus2_50s`. Για διαφορετικά paths μπορεί να δοθεί ρητά input και output, για παράδειγμα `python IEEE39/analyze_ieee39.py --scenario load29_p4 --data-dir results/Load29_Pplus4_50s --output-dir analysis/Load29_Pplus4_50s`. Όταν χρησιμοποιείται `--data-dir`, το όνομα που δίνεται στο `--scenario` είναι απλώς label για το run και δεν χρειάζεται να είναι προκαθορισμένο key. Στο `analyze_ieee39.py`, τα `--data-dir`, `--output-dir` και `--analysis-dir` δέχονται relative paths relative προς τον φάκελο `IEEE39`, ή absolute paths.
